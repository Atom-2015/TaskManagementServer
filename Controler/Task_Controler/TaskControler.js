...
module.exports.HandleTaskCreation = async (req, res) => {
  ...
  try {
    ...
    project.tasks.push(newTask._id);
    await project.save();
    const user = await User.findById(taskAssign); 
    if (!user) {
    ...
    const payload = { useremail, name };

    let emailsent = false;
    try {
        var sendtaskmail = await axios.post('http://127.0.0.1:3001/api/mailTask', payload);

      emailsent = true;
      console.log("Email sent response:", sendtaskmail.data);
    } catch (err) {
      console.error("Failed to send email:", err.message);
    }

    console.log(`this email sent data ${sendtaskmail}`);
    return res.status(201).json({
      message: "Task created successfully",
      status: true,
      data: newTask,
...
module.exports.HandleTaskAssignedToUser = async (req, res) => {
  ...
  try {
    ...
    const task = await Task.find({ assigned_to: userid._id }).populate(
      "project_id"
    );
